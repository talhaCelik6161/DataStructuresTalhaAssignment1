...
public class HeapPriorityQueue<K, V> extends AbstractPriorityQueue<K, V> {
    ...
    public HeapPriorityQueue(K[] keys, V[] values) {
        this(); // Call default constructor to initialize the comparator
    ...
    protected void upheap(int j) {
        while (j > 0) {
            int p = parent(j);
            if (compare(heap.get(j), heap.get(p)) >= 0) break; // Heap property is satisfied
            swap(j, p);
            j = p;
        }
        // TODO
    ...
    protected void downheap(int j) {
        while (hasLeft(j)) {
            ...
            if (compare(heap.get(smallChildIndex), heap.get(j)) >= 0) break; // Heap property is satisfied
